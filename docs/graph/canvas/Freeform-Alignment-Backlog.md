...
| **F-11** | **selection 边框 zoom 偏移 bug** | **P0** | 小-中 | zoom 后 selection 与 mesh 不贴合(M2.2 后显眼) |
...
### F-11 selection 边框 zoom 偏移 bug(P0,小-中)

- **现象**:画板 zoom in/out 后,选中态的蓝色矩形边框与 sticky / 文字节点
  渲染 mesh **不贴合**(虚框留在世界坐标的旧 size,mesh 也用世界坐标但视觉
  上看起来"分离了")
- **历史**:M2.1.6abc 之前就存在,但深色背景 + 浅文字下不易察觉;**M2.2
  Sticky 黄底显眼后**才被看到 — 不是 Sticky 引入的 bug
- **诊断方向**:
  - selection LineLoop 用 `node.position + node.size` 算 4 顶点(世界坐标);
    mesh 通过 `outer.position = position + size/2` 定位,理论应一致
  - **怀疑**:zoom 触发 mesh.matrixWorld 重算 → `adaptTextNodeSizeToContent`
    用 setFromObject 测量 SVG bbox 时可能拿到错误高度 → 错误改 `node.size.h`
    → selection 重算用了错值
  - 或:selection LineLoop 在 zoom 时**没被 refreshOverlays 调用**
- **抓取**:
  - 重现条件:创建 sticky/文字节点 → 双击编辑/退出 → 然后 zoom in/out 即触发
  - 加诊断 log:在 `refreshOverlays` 入口打 node.position/size + group.position;
    在 `adaptTextNodeSizeToContent` 打 oldH/newH/contentH;比对哪一边数据漂移
- **修复方向(待诊断后定)**:
  - 如果是 adapt 错改 size:加防御 `Math.abs(newH - oldH) < threshold` 或
    限制 adapt 只在编辑后调一次
  - 如果是 selection 没刷新:zoom 监听里加 refreshOverlays 调用
- **复杂度**:小-中,等诊断结果定
- **价值**:**P0** — 视觉错位严重影响 sticky 体验;不修无法 ship M2.2
