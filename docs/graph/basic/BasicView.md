...
4. **v1 范围本来就小**:5 个 shape + 1-2 种 label 布局 + 基础 SceneManager / InteractionController + 注册中心接口,从零写约 1.5-2 天。
...
BasicView v1 单独的工作量(从零构建,不带历史包袱):
...
- M1d: InteractionController(pan / zoom / 单选 / drag)+ Substance 注册中心(family/person 等内置)— **0.5 天**

合计 BasicView v1: **1.5-2 天**(family-tree variant 驱动 + 验证)
