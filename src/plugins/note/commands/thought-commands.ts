 ...
 *
 * 流程：
 * 1. 先在 DB 中创建 Thought 记录（获取真实 ID）
 * 2. 再将该 ID 写入 ProseMirror mark / node attr
 * 3. 打开 Right Slot（如果尚未打开）
 * 4. 通知 Thought 面板（延迟发送，等待 ThoughtView 加载）
 *
 * 三条路径：
 * 1. 有文字选择 → inline mark
...
export async function addThought(
  ...
  if (nodeAtPos && nodeAtPos.type.spec.attrs?.thoughtId !== undefined) {
    ...
    const mark = thoughtMarkType.create({ thoughtId: record.id, thoughtType: type });
    ...
    anchorText = state.doc.textBetween(blockStart, blockEnd, ' ').slice(0, 100);
    anchorPos = blockStart;

    const record = await createRecord(api, noteId, anchorType, anchorText, anchorPos, type);
    if (!record) return;

    const mark = thoughtMarkType.create({ thoughtId: record.id, thoughtType: type });
    const tr = state.tr.addMark(blockStart, blockEnd, mark);
    view.dispatch(tr);
