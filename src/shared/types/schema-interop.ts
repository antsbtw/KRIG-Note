/**
 * 互操作性层 Schema (Interoperability Layer)
 * 
...
/**
 * 彻底取代原来的 app.ts openRightSlot()
 * L5想开新窗口，必须使用这个接口发送，由框架决定行为。
 */
export interface IntentPayload<T = unknown> {
  action: 'open-content' | 'ask-ai' | 'search' | 'split-screen';
  sourceViewId: string;            // 发起者
  data: T;                         // 具体参数，如 { contentType: 'note', id: '123' }
}

// ── 2. 转换器契约 (Converters) ──

export interface IRepresentationConverter<TRenderData> {
  toAtom(data: TRenderData, provenance: import('./schema-semantic').Provenance): import('./schema-representation').Atom[];
  fromAtom(atoms: import('./schema-representation').Atom[]): TRenderData;
}
